...
export function detectConversationPhase(rules: StrategyRule[]): ConversationPhase {
  const hasEntry = rules.some(r => r.category === 'entry' || r.label.toLowerCase().includes('entry'));
  const hasStop = rules.some(r => r.label.toLowerCase().includes('stop') && !r.label.toLowerCase().includes('time'));
  const hasTarget = rules.some(r => r.label.toLowerCase().includes('target') || r.label.toLowerCase().includes('profit'));
  const hasSize = rules.some(r => r.label.toLowerCase().includes('position') || r.label.toLowerCase().includes('size'));
...
export function detectCurrentFocus(message: string): string {
  ...
  if (lower.includes('entry') || lower.includes('trigger') || lower.includes('signal')) return 'entry';
  if (lower.includes('stop') && !lower.includes('time')) return 'stop';
  if (lower.includes('target') || lower.includes('profit') || lower.includes('take profit')) return 'target';
  if (lower.includes('size') || lower.includes('position') || lower.includes('contracts')) return 'sizing';
  if (lower.includes('timeframe') || lower.includes('chart')) return 'timeframe';
  if (lower.includes('session') || lower.includes('hours')) return 'session';
  if (lower.includes('filter') || lower.includes('condition')) return 'filters';
...
export function detectMistakes(rules: StrategyRule[]): string[] {
  ...
  if (!hasStop && rules.length > 2) {
    issues.push(COMMON_MISTAKES.noStopLoss.fix);
  }
  
  // Check for excessive risk
  const sizeRules = rules.filter(r => r.label.toLowerCase().includes('risk') && r.value.includes('%'));
...
export function generateContextualPrompt(context: ConversationContext): string {
  const { phase, rules, lastUserMessage, missingComponents } = context;
  ...
  switch (phase) {
    ...
    case 'sizing_definition':
      return generateSizingPrompt(rules);
...
"What kind of setups do you look for on ${'{instrument}'}? For example:
...
- Avoid multiple-choice questions that limit their description
...
function generateEntryPrompt(lastMessage: string): string {
  const focus = detectCurrentFocus(lastMessage);
...
**Subtypes:**
${JSON.stringify(taxonomy.subtypes, null, 2)}

**Common Mistakes:**
${taxonomy.commonMistakes?.map(m => `• ${m}`).join('\n')}
...
${COMMON_MISTAKES.noStopLoss.professionalQuote}
...
function generateSizingPrompt(rules: StrategyRule[]): string {
  return `
...
3. **Validate against prop firm standards**:
   ${Object.entries(PROP_FIRM_STANDARDS).map(([firm, rules]) => 
     `• ${firm}: Daily loss ${rules.dailyLoss || 'none'}, Max contracts ${JSON.stringify(rules.contracts)}`
   ).join('\n   ')}
...
function detectEntryType(text: string): keyof typeof ENTRY_TAXONOMY | null {
  const lower = text.toLowerCase();
  
  if (lower.includes('breakout') || lower.includes('break above') || lower.includes('break below')) {
    return 'breakout';
  }
  if (lower.includes('pullback') || lower.includes('retest') || lower.includes('retrace')) {
    return 'pullback';
  }
  if (lower.includes('reversal') || lower.includes('pivot')) {
    return 'reversal';
  }
  ...
  if (lower.includes('confirm') || lower.includes('signal')) {
    return 'confirmation';
  }
...
export function getRelevantExamples(focus: string, entryType?: string): string[] {
  if (focus === 'entry' && entryType) {
    ...
    if (taxonomy && 'subtypes' in taxonomy) {
      return Object.values(taxonomy.subtypes as any).map((st: any) => st.example).filter(Boolean);
    }
  }
...
export function getCalculationHelp(rules: StrategyRule[]): string {
  const stopRule = rules.find(r => r.label.toLowerCase().includes('stop'));
  const sizeRule = rules.find(r => r.label.toLowerCase().includes('size') || r.label.toLowerCase().includes('risk'));
