...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  ...
  private static getMissingComponents(rules: StrategyRule[]): string[] {
    ...
    const hasInstrument = rules.some(r => r.label.toLowerCase().includes('instrument') || r.label.toLowerCase().includes('symbol'));
   ...
   */
  static validateResponse(response: string, context: ConversationContext): {
    valid: boolean;
    issues: string[];
  } {
    const issues: string[] = [];
 ...
 */
export function detectCriticalErrors(rules: StrategyRule[]): Array<{
  severity: 'critical' | 'warning';
  message: string;
  fix: string;
}> {
  const errors: Array<{ severity: 'critical' | 'warning'; message: string; fix: string }> = [];
  
  // Check for no stop loss
  const hasStop = rules.some(r => r.label.toLowerCase().includes('stop'));
  if (!hasStop && rules.length > 3) {
    errors.push({
      severity: 'critical',
  ...
  const riskRules = rules.filter(r => r.value.includes('%') && r.label.toLowerCase().includes('risk'));
  ...
  for (const rule of entryRules) {
    const vagueTerms = ['looks good', 'feels right', 'depends', 'maybe', 'probably'];
  ...
  if (targetRule && targetRule.value.includes('1:1')) {
    errors.push({
      severity: 'warning',
...
/**
 * Example of how to use this skill in your chat handler
 */
export function exampleUsage() {
  ...
  const lastUserMessage = 'I want to trade NQ breakouts';
  const basePrompt = 'You are a trading strategy assistant...';
  ...
  const nextAction = TradingIntelligenceSkill.getNextAction(rules);
