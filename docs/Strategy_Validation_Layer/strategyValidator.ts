...
// ============================================================================
// VALIDATION RULES (Based on Professional Standards)
// ============================================================================
 ...
 */
function validateEntry(entry: EntryComponent, allRules: StrategyRule[]): ValidationIssue[] {
 ...
 */
function validateRiskReward(stop: ExitComponent, target: ExitComponent, allRules: StrategyRule[]): ValidationIssue[] {
  const issues: ValidationIssue[] = [];
  ...
  if (rrMatch) {
    ...
    if (ratio >= PROFESSIONAL_STANDARDS.recommendedRR) {
      // This is good! No issue, but we could add a positive note
    }
  }
...
export function getNextActionSuggestion(validation: ValidationResult): string {
  if (validation.requiredMissing.length > 0) {
    return `Define: ${validation.requiredMissing[0].field}`;
  }
  
  if (validation.errors.length > 0) {
    return `Fix: ${validation.errors[0].field}`;
  }
  
  if (validation.recommendedMissing.length > 0) {
    return `Consider adding: ${validation.recommendedMissing[0].field}`;
  }
